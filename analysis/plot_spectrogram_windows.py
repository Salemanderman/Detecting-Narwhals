...
def plot_window_grid(
    ...
    for idx, (start_sec, win) in enumerate(windows):
        r, c = divmod(idx, cols)
        ax = axes[r][c]
...
def main():
    ...
    ap.add_argument("--mel-end",      type=int,   default=128,  help="Last mel bin (exclusive) to include (default: 128).")
    ap.add_argument("--max-per-file", type=int,   default=20,
                    help="Max windows to plot per file (default: 20). Use 0 for all.")
    ap.add_argument("--cols",         type=int,   default=4,    help="Grid columns per plot (default: 4).")
    ...
    print(f"Max per file:  {args.max_per_file if args.max_per_file > 0 else 'all'}")
    ...
    for npz_path in npz_files:
        ...
        all_windows = [
            (round(start_frame * secs_per_frame, 3), win)
        ...
        if args.max_per_file > 0:
            selected = all_windows[: args.max_per_file]
        else:
        ...
        plot_window_grid(
            windows        = selected,
            title          = f"{npz_path.name}  |  {len(selected)} of {len(all_windows)} windows  "
                             f"({window_secs:.1f} s each, mel {mel_start}–{mel_end - 1})",
        ...
        print(f"  {npz_path.name}: {len(all_windows)} windows total, "
              f"plotted {len(selected)}  ->  {out_path.name}")
